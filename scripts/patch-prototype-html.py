#!/usr/bin/env python3
"""Apply brochure sync updates to prototype HTML."""

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
AR = ROOT / "public/prototype/index.html"
EN = ROOT / "public/prototype/en/index.html"
...
def replace_eco_grid(text: str) -> str:
    start = text.index('    <div class="eco-grid fade-in">')
    end = text.index("<!-- ============================================================\n     7 · THE PLACE")
    return text[:start] + ECO_GRID + "\n  </div>\n</section>\n\n" + text[end:]


def patch(path: Path, location: str, is_ar: bool) -> None:
    t = path.read_text(encoding="utf-8")
    t = replace_eco_grid(t)
    ...
    dev_pos = t.find('class="dev-img"')
    if dev_pos >= 0:
        t = t[:dev_pos] + t[dev_pos:].replace("hospitality.webp", "cover.webp", 1)
    ...
    if is_ar:
        t = t.replace(
            "مشروع بالم هيلز الأول كمدينة ساحلية متكاملة على كيلo ٢٣٨",
            "مشروع بالم هيلز الأول كمدينة ساحlية مسوّرة على كيلo ٢٣٨",
        )
        t = t.replace(
        ...
        t = t.replace("<h3 class=\"feature-title\">فيلا فائقة الفخامة</h3>", "<h3 class=\"feature-title\">فيلا فائقة الفخامة — Shoreline Villa</h3>")
        ...
        t = t.replace(
            ...
            "<span class=\"office-area\">5th Settlement · التجمع الخامs</span>",
        )
    else:
    ...
    t = t.replace("background-image:url('/hacienda/brochure/chalet.png')", "background-image:url('/hacienda/brochure/unit-chalet.webp')")
    t = t.replace(
    ...
    meta_map = {
        '<span class="unit-meta">Shoreline view</span>': '<span class="unit-meta">Seabreeze · '
        + ("حتى 280 m²" if is_ar else "up to 280 m²")
        + "</span>",
        '<span class="unit-meta">Branded Residences</span>': '<span class="unit-meta">Beach Homes · '
        + ("من 65 m²" if is_ar else "from 65 m²")
        + "</span>",
        '<span class="unit-meta">Garden &amp; landscape</span>': '<span class="unit-meta">The Duo · 320 m² land</span>',
    }
...
def main() -> None:
    patch(AR, LOCATION_AR, True)
    patch(EN, LOCATION_EN, False)
